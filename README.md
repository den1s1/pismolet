...
- Sprint 9 inbound replies и fake-пересылка ответа клиенту;
- Sprint 10 admin-контроль: клиенты, блокировки, лимиты, настройки MVP, audit log, импорты, complaints, delivery errors, replies;
...
- админ-зона: `/admin`;
- клиенты Sprint 10: `/admin/clients`;
- настройки MVP Sprint 10: `/admin/settings/mvp`;
- audit log Sprint 10: `/admin/audit`;
- импорты Sprint 10: `/admin/imports`;
- жалобы Sprint 10: `/admin/complaints`;
- ошибки доставки Sprint 10: `/admin/delivery-errors`;
- ответы Sprint 10: `/admin/replies`;
- очередь модерации: `/admin/moderation`;
- изменение дневного лимита клиента: `/admin/limits` или `/admin/clients`;
...
Дневной лимит берётся из `ClientProfile.DailySendLimit`. Изменить его можно через `/admin/limits` или `/admin/clients`. Для MVP-дня используется UTC-день.
...
1. Задать admin allowlist через `Admin:AllowedEmails` или `PISMOLET_ADMIN_EMAILS`.
2. Проверить, что обычный пользователь не открывает `/admin` и не выполняет `/admin/*` POST actions.
3. Открыть `/admin` администратором.
4. Открыть `/admin/clients`, изменить лимит клиента и увидеть запись `admin_client_daily_limit_changed` в `/admin/audit`.
5. Заблокировать клиента и убедиться, что новая оплата/отправка не проходит backend-проверки.
6. Включить клиенту премодерацию и проверить следующий risk-flow.
7. Открыть `/admin/settings/mvp`, изменить цену письма и проверить новый расчёт оплаты.
8. Открыть `/admin/complaints`, `/admin/delivery-errors`, `/admin/replies`, `/admin/imports`.

Ограничение текущего среза: price settings для Postgres пока применяются в runtime-репозитории и `IPriceSettingsRepository`; отдельная EF-таблица настроек остаётся следующим hardening-срезом.
