...
{
    public bool TryAdd(Mailing m) { if (db.Mailings.Any(x => x.Id == m.Id)) return false; db.Mailings.Add(ToEntity(m)); SaveOwned(m); db.SaveChanges(); return true; }
    ...
    public void Update(Mailing m) { var e = db.Mailings.FirstOrDefault(x => x.Id == m.Id); if (e is null) db.Mailings.Add(ToEntity(m)); else { e.OwnerEmail = N(m.OwnerEmail); e.Subject = m.Subject; e.StatusRu = m.StatusRu; e.PublicId = m.PublicId; e.CreatedAt = m.CreatedAt.ToUniversalTime(); } ClearOwned(m.Id); SaveOwned(m); db.SaveChanges(); }
    ...
    private void ClearOwned(Guid id) { var batchIds = db.ImportBatches.Where(x => x.MailingId == id).Select(x => x.Id).ToArray(); db.ImportIssues.RemoveRange(db.ImportIssues.Where(x => batchIds.Contains(x.ImportBatchId))); db.Recipients.RemoveRange(db.Recipients.Where(x => x.MailingId == id)); db.ImportBatches.RemoveRange(db.ImportBatches.Where(x => x.MailingId == id)); db.MailingDeclarations.RemoveRange(db.MailingDeclarations.Where(x => x.MailingId == id)); db.MailingMessageDrafts.RemoveRange(db.MailingMessageDrafts.Where(x => x.MailingId == id)); }
    private void SaveOwned(Mailing m) { foreach (var b in m.ImportBatches) { db.ImportBatches.Add(new ImportBatchEntity { Id = b.Id == Guid.Empty ? Guid.NewGuid() : b.Id, MailingId = m.Id, FileName = b.FileName, SourceFormat = b.SourceFormat.ToString(), CreatedAt = b.CreatedAt.ToUniversalTime(), TotalRows = b.TotalRows, Accepted = b.Accepted, Duplicates = b.Duplicates, Invalid = b.Invalid, GloballySuppressed = b.GloballySuppressed, ClientSuppressed = b.ClientSuppressed, Status = b.Status.ToString() }); db.ImportIssues.AddRange(b.Issues.Select(issue => new ImportIssueEntity { Id = Guid.NewGuid(), ImportBatchId = b.Id, RowNumber = issue.RowNumber, Email = issue.Email, Message = issue.Message })); } db.Recipients.AddRange(m.Recipients.Select(r => new RecipientEntity { Id = Guid.NewGuid(), MailingId = m.Id, SourceEmail = r.SourceEmail, NormalizedEmail = N(r.Email), Status = r.Status.ToString(), ExclusionReason = r.ExclusionReason, ImportBatchId = r.ImportBatchId })); if (m.Declaration is { } d) db.MailingDeclarations.Add(new MailingDeclarationEntity { MailingId = m.Id, ImportBatchId = d.ImportBatchId, UserEmail = N(d.UserEmail), BaseSource = d.BaseSource.ToString(), IsBaseLegalityConfirmed = d.IsBaseLegalityConfirmed, IsAdvertisingConsentConfirmed = d.IsAdvertisingConsentConfirmed, DeclarationVersion = d.DeclarationVersion, CreatedAt = d.CreatedAt.ToUniversalTime(), Ip = d.Ip, UserAgent = d.UserAgent }); if (m.MessageDraft is { } dr) db.MailingMessageDrafts.Add(new MailingMessageDraftEntity { MailingId = m.Id, SenderName = dr.SenderName, Subject = dr.Subject, Body = dr.Body, MessageType = dr.MessageType.ToString(), UpdatedAt = dr.UpdatedAt.ToUniversalTime() }); }
    private Mailing ToDomain(MailingEntity e) { var issueEntities = db.ImportIssues.AsNoTracking().Join(db.ImportBatches.AsNoTracking().Where(x => x.MailingId == e.Id), issue => issue.ImportBatchId, batch => batch.Id, (issue, batch) => issue).ToArray(); var issuesByBatch = issueEntities.GroupBy(x => x.ImportBatchId).ToDictionary(x => x.Key, x => (IReadOnlyCollection<RecipientImportIssue>)x.OrderBy(issue => issue.RowNumber).Select(issue => new RecipientImportIssue(issue.RowNumber, issue.Email, issue.Message)).ToArray()); var batches = db.ImportBatches.AsNoTracking().Where(x => x.MailingId == e.Id).OrderBy(x => x.CreatedAt).ToArray().Select(x => new ImportBatch(x.Id, x.MailingId, x.FileName, Enum.Parse<ImportSourceFormat>(x.SourceFormat), x.CreatedAt, x.TotalRows, x.Accepted, x.Duplicates, x.Invalid, x.GloballySuppressed, Enum.Parse<ImportBatchStatus>(x.Status), issuesByBatch.GetValueOrDefault(x.Id) ?? Array.Empty<RecipientImportIssue>(), x.ClientSuppressed)).ToList(); var recipients = db.Recipients.AsNoTracking().Where(x => x.MailingId == e.Id).ToArray().Select((x, index) => new Recipient(x.SourceEmail, x.NormalizedEmail, Enum.Parse<RecipientStatus>(x.Status), x.ExclusionReason, x.ImportBatchId, index + 2)).ToList(); var declaration = db.MailingDeclarations.AsNoTracking().FirstOrDefault(x => x.MailingId == e.Id); var draft = db.MailingMessageDrafts.AsNoTracking().FirstOrDefault(x => x.MailingId == e.Id); var last = batches.OrderByDescending(x => x.CreatedAt).FirstOrDefault(); return new Mailing(e.Subject, e.StatusRu) { Id = e.Id, OwnerEmail = e.OwnerEmail, CreatedAt = e.CreatedAt, PublicId = e.PublicId, ImportBatches = batches, LastImportBatch = last, LastImportStats = last?.ToStats() ?? ImportStats.Empty, Recipients = recipients, Declaration = declaration is null ? null : new MailingDeclaration(declaration.MailingId, declaration.UserEmail, Enum.Parse<BaseSource>(declaration.BaseSource), declaration.IsBaseLegalityConfirmed, declaration.IsAdvertisingConsentConfirmed, declaration.DeclarationVersion, declaration.CreatedAt, declaration.Ip, declaration.UserAgent) { ImportBatchId = declaration.ImportBatchId }, MessageDraft = draft is null ? null : new MailingMessageDraft(draft.SenderName, draft.Subject, draft.Body, Enum.Parse<MessageType>(draft.MessageType), draft.UpdatedAt) }; }
    private static string N(string v) => v.Trim().ToLowerInvariant();
}
