    ...
    IGlobalSuppressionRepository suppressions) : IAdminRecipientRepository
{
    public IReadOnlyCollection<AdminRecipientSummary> ListSummaries()
    {
        ...
        return mailingList
            ...
            .Select(group => ToSummary(group.Key, group, suppressions.GetByEmail(group.Key), Array.Empty<SendSnapshot>()))
    ...
    public AdminRecipientProfile? GetProfile(string email)
    {
        ...
        return ToProfile(normalized, snapshots, suppressions.GetByEmail(normalized), Array.Empty<SendSnapshot>());
    }
}
...
public sealed class EfAdminRecipientRepository(PismoletDbContext db) : IAdminRecipientRepository
{
    ...
    public AdminRecipientProfile? GetProfile(string email)
    {
        ...
        return ToProfile(normalized, recipients, suppression, LoadSendEvents([normalized]));
    }
    ...
    private RecipientSnapshot[] LoadRecipients(string? normalizedEmail)
    {
        var query = from recipient in db.Recipients.AsNoTracking()
                    ...
                    where recipient.Status == RecipientStatus.Accepted.ToString()
                    select new RecipientSnapshot(
                        recipient.NormalizedEmail,
                        mailing.Id,
                        mailing.OwnerEmail,
                        mailing.Subject,
                        MailingStatusLabels.FromRu(mailing.StatusRu),
                        db.Recipients.Count(x => x.MailingId == mailing.Id),
                        mailing.CreatedAt,
                        null);
...
file static AdminRecipientSummary ToSummary(
    string email,
...
file static AdminRecipientProfile ToProfile(
    string email,
    IEnumerable<RecipientSnapshot> recipientRows,
    GlobalSuppression? suppression,
    IEnumerable<SendSnapshot> sendRows)
{
    ...
    return new AdminRecipientProfile(summary, owners, mailings);
}

file static (string Code, string Text) ResolveStatus(GlobalSuppression? suppression, IReadOnlyCollection<SendSnapshot> sends)
{
...
file static string Normalize(string email) => email.Trim().ToLowerInvariant();
