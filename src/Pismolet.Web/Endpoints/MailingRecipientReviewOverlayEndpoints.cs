...
public static class MailingRecipientReviewOverlayEndpoints
{
    ...
    private static async Task<IResult> ImportRecipients(Guid id, HttpContext http, IMailingService mailings, IRecipientImportService imports, IMailingDeclarationService declarations, CancellationToken cancellationToken)
    {
        ...
        {
            var declaration = declarations.Confirm(new ConfirmMailingDeclarationCommand(email, id, TryParseBaseSource(form["baseSource"].ToString()), form.ContainsKey("baseLegality"), form.ContainsKey("advertisingConsent"), TryParseMessageType(form["messageType"].ToString()), ToRequestMetadata(http)));
            return declaration.Ok
                ? Results.Redirect($"/mailings/{id}/message")
                : Page("Адресаты", RecipientReviewPage(declaration.Mailing ?? current, string.Empty, declaration.Error));
        }
...
<section class='wizard-shell confirmation-step'>
  ...
  <section class='panel'>
    ...
    private static bool HasDeclarationFields(IFormCollection form) => form.ContainsKey("baseSource") || form.ContainsKey("baseLegality") || form.ContainsKey("messageType") || form.ContainsKey("advertisingConsent");
