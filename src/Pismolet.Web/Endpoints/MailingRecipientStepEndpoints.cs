 ...
 <section class='wizard-shell confirmation-step payment-wizard'>
   ...
   <section class='panel'>
     ...
     <form method='post' action='/mailings/{mailing.Id}/payment/start' class='compact-base-form address-declaration-form confirmation-payment-form' aria-label='Финальное подтверждение'>
       ...
       <label class='compact-base-check'><input type='checkbox' name='baseLegality'{baseChecked} required><span>подтверждаю правомерность использования базы и <a href='/legal/data-processing?returnUrl=/mailings/{mailing.Id}/confirmation'>поручаю техническую обработку email-адресов</a></span></label>
