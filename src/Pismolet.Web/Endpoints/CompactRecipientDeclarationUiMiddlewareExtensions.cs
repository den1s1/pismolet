...
public static class CompactRecipientDeclarationUiMiddlewareExtensions
{
    ...
    private static string CompactForm(string action, string legalHref) => $@"
<form method='post' action='{action}' class='compact-base-form' style='display:grid;gap:10px;margin-top:10px;max-width:620px;'>
  <div class='compact-base-fields' style='display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;'>
    <label class='compact-base-field' style='display:grid;gap:5px;flex:0 0 280px;font-weight:400;border:0;background:transparent;padding:0;'><span style='font-weight:400;'>Источник базы</span><select name='baseSource' required style='width:100%;min-height:38px;padding:7px 10px;border-radius:12px;font-weight:400;'><option value='' style='font-weight:400;'>Выберите источник</option></select></label>
