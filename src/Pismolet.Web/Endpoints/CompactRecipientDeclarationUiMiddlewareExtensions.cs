...
{
    private static readonly Regex DeclarationFormRegex = new(@"<form method='post' action='(?<action>[^']+/declaration)' class='form-grid confirmation-list'>.*?</form>", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.CultureInvariant);
    private static readonly Regex LegalBoxRegex = new(@"\s*<section class='box muted-box'>\s*<h2>Декларация законности базы</h2>\s*<p>Полный текст вынесен в отдельный юридический документ\.</p>\s*<a class='btn secondary' href='(?<href>[^']+)'>Открыть декларацию</a>\s*</section>", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.CultureInvariant);
    private static readonly Regex ExcludedBlockRegex = new(@"\s*<h2>Что исключено</h2>\s*(?:<p class='muted'>Исключённых адресов нет\.</p>|<ul class='issue-list'>.*?</ul>)", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.CultureInvariant);
    private static readonly Regex BaseCardStartRegex = new(@"<section class='box'>\s*<h2>Подтвердите базу</h2>", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.CultureInvariant);
    ...
    private static bool ShouldTransform(PathString path)
    {
        ...
        return value.StartsWith("/mailings/", StringComparison.OrdinalIgnoreCase) && value.EndsWith("/recipients", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsHtml(string? contentType) => contentType?.Contains("text/html", StringComparison.OrdinalIgnoreCase) == true;
    ...
    private static string Transform(string html, string recipientsPath)
    {
        ...
        html = DeclarationFormRegex.Replace(html, match => CompactForm(match.Groups["action"].Value, legalHref));
    ...
    private static string AddScript(string html) => html.Contains("compact-ad-consent-script", StringComparison.Ordinal) ? html : html.Replace("</body>", AdvertisingConsentScript() + "\n</body>", StringComparison.OrdinalIgnoreCase);

    private static string InitialControls(string legalHref) => $"<section class='inline-base-confirm' style='grid-column:1/-1;margin-top:8px;'><h2>Подтвердите базу</h2>{Controls(legalHref)}</section>";
    private static string CompactForm(string action, string legalHref) => $"<form method='post' action='{action}' class='compact-base-form' style='display:grid;gap:10px;margin-top:10px;max-width:620px;'>{Controls(legalHref)}<button class='button' style='width:max-content;min-width:190px;margin-top:2px;'>Перейти к письму</button></form>";

    private static string Controls(string legalHref)
    {
        var sourceOptions = string.Join("", BaseSourceLabels.All.Select(x => $"<option value='{x.Key}' style='font-weight:400;'>{x.Value}</option>"));
        return $"<div class='compact-base-fields' style='display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;'><label class='compact-base-field' style='display:grid;gap:5px;flex:0 0 280px;font-weight:400;border:0;background:transparent;padding:0;'><span style='font-weight:400;'>Источник базы</span><select name='baseSource' required style='width:100%;min-height:38px;padding:7px 10px;border-radius:12px;font-weight:400;'><option value='' style='font-weight:400;'>Выберите источник</option>{sourceOptions}</select></label><label class='compact-base-field' style='display:grid;gap:5px;flex:0 0 220px;font-weight:400;border:0;background:transparent;padding:0;'><span style='font-weight:400;'>Тип письма</span><select name='messageType' id='messageTypeSelect' style='width:100%;min-height:38px;padding:7px 10px;border-radius:12px;font-weight:400;'><option value='Transactional' style='font-weight:400;'>Информационное</option><option value='Advertising' style='font-weight:400;'>Рекламное</option></select></label></div><label class='compact-base-check' style='display:flex;align-items:center;gap:8px;font-weight:400;border:0;background:transparent;padding:0;margin:0;'><input type='checkbox' name='baseLegality' style='width:16px;height:16px;min-width:16px;margin:0;'><span style='font-weight:400;'>подтверждаю правомерность использования базы</span></label><label class='compact-base-check compact-ad-consent' id='advertisingConsentBlock' style='display:none;align-items:center;gap:8px;font-weight:400;border:0;background:transparent;padding:0;margin:0;'><input type='checkbox' name='advertisingConsent' style='width:16px;height:16px;min-width:16px;margin:0;'><span style='font-weight:400;'>подтверждаю наличие рекламного согласия адресатов</span></label><p class='compact-legal-link' style='margin:2px 0 0;'><a href='{legalHref}' style='font-weight:400;'>Декларация законности базы</a></p>";
    }
