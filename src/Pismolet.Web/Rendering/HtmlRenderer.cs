...
public static class HtmlRenderer
{
    ...
    public static string AccountForm(string action, string title, bool name, bool password = true, bool registrationConsents = false)
    {
        ...
        var consentFields = registrationConsents ? """
            <div class='form-consents checkbox-list'>
              <label class='checkbox-row'><input class='checkbox-control' type='checkbox' name='acceptOffer' value='true' required><span class='checkbox-text'>Принимаю <a href='/legal/offer?returnUrl=/account/register'>оферту</a> и <a href='/legal/rules?returnUrl=/account/register'>правила рассылок</a>.</span></label>
              <label class='checkbox-row'><input class='checkbox-control' type='checkbox' name='acceptPrivacy' value='true' required><span class='checkbox-text'>Даю <a href='/legal/client-consent?returnUrl=/account/register'>согласие на обработку моих персональных данных</a> как пользователя Письмолёта: email, ФИО, телефон, сведения профиля, данные об оплатах, действиях в сервисе и обращениях в поддержку. Согласие нужно для регистрации, работы сервиса, оплаты, поддержки, безопасности и исполнения договора. Подробнее: <a href='/legal/privacy?returnUrl=/account/register'>политика обработки персональных данных</a>.</span></label>
            </div>
            """ : string.Empty;
            ...
            <section class='dashboard-shell'>
                <section class='panel quick-start'>
                    <div>
                        ...
                        <div class='actions'><a class='btn' href='/mailings/new'>Создать рассылку</a><a class='btn secondary' href='#mailing-history'>История рассылок</a></div>
                    </div>
                </section>
                <section class='panel'>
                    <div class='section-head'><div><p class='eyebrow'>Аккаунт</p><h2>Состояние отправки</h2></div><span class='badge neutral'>{H(user.Profile.Status)}</span></div>
                    <div class='stats'><div class='stat'><b>{total}</b><span>Всего рассылок</span></div><div class='stat'><b>{active}</b><span>В работе</span></div><div class='stat'><b>{sent}</b><span>Отправлено</span></div><div class='stat'><b>{user.Profile.DailySendLimit}</b><span>Дневной лимит</span></div></div>
                </section>
                <section class='panel' id='mailing-history'>
                    <div class='section-head'><div><p class='eyebrow'>Контроль</p><h2>История рассылок</h2></div><a class='btn secondary' href='/mailings/new'>Новая рассылка</a></div>
    ...
    public static string Payments(UserAccount user)
    {
        ...
        return $"""
            <section class='cabinet-grid'>
                ...
                <section class='panel'><div class='section-head'><div><p class='eyebrow'>Оплачено</p><h2>Оплаченные рассылки</h2></div></div><div class='history-list'>{PaymentRows(paid, "Пока нет оплаченных рассылок.")}</div></section>
    ...
    public static string EmailPreview(EmailMessage message) => $"""
        <div class='card'><h1>{H(message.Subject)}</h1><p><strong>Кому:</strong> {H(message.Recipient.Email)}</p><p><strong>Тема:</strong> {H(message.Subject)}</p><pre>{H(message.PlainTextBody)}</pre></div>
        """;
    ...
    public static string FakeMailer(IEnumerable<FakeMail> messages)
    {
        ...
        var rows = list.Count == 0
            ...
            : string.Join(string.Empty, list.Select(message => $"""
                <div class='history-row'><div><b>{H(message.Subject)}</b><br><span class='hint'>Кому: {H(message.To)}</span></div><a class='text-link' href='{H(message.Link)}'>Открыть</a></div>
                """));
        return $"""
    ...
    private static string MailingHistoryRows(IReadOnlyCollection<Mailing> mailings)
    {
        ...
        var rows = string.Join(string.Empty, mailings.Select(mailing =>
        {
            ...
            return $"""
                <div class='history-row campaign-history-row'><div class='campaign-main'><b>{H(MailingTitle(mailing))}</b><span class='hint'>Создано: {mailing.CreatedAt:dd.MM.yyyy}</span></div><div><span class='history-label'>Письма</span><b>{accepted}</b></div><div><span class='history-label'>Стоимость</span><b>{estimatedCost:0.##} ₽</b></div><div><span class='history-label'>Статус</span><span class='{BadgeClass(mailing)}'>{H(mailing.StatusRu)}</span></div><div class='history-actions'><a class='text-link' href='{H(actionUrl)}'>{H(actionText)}</a></div></div>
                """;
        }));
        return $"<div class='history-table'><div class='history-table-head'><span>Тема письма</span><span>Количество писем</span><span>Стоимость</span><span>Статус</span><span>Действие</span></div>{rows}</div>";
    }
    ...
    private static string PaymentRows(IReadOnlyCollection<Mailing> mailings, string emptyText)
    {
        ...
        return string.Join(string.Empty, mailings.Select(m => $"""
            <div class='history-row'><div><b>{H(MailingTitle(m))}</b><br><span class='hint'>Статус: {H(m.StatusRu)}</span></div><div class='history-actions'><span class='{BadgeClass(m)}'>{H(m.StatusRu)}</span><a class='text-link' href='{H(DashboardAction(m).Url)}'>Открыть</a></div></div>
            """));
    }
    ...
    private static string PublicNav() => """
        <div class='account'><a class='btn secondary' href='/account/login'>Войти</a><a class='btn' href='/account/register'>Регистрация</a></div>
        """;

    private static (string Url, string Text) DashboardAction(Mailing mailing)
    {
        if (mailing.LastImportStats.Accepted <= 0) return ($"/mailings/{mailing.Id}/recipients", "Продолжить");
        if (mailing.Declaration is null) return ($"/mailings/{mailing.Id}/declaration", "Подтвердить базу");
    ...
    private static string MailingTitle(Mailing mailing) => string.IsNullOrWhiteSpace(mailing.MessageDraft?.Subject) ? "Новая рассылка" : mailing.MessageDraft!.Subject;
