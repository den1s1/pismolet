...
public sealed class AdminOperationService(IUserRepository users, IMailingRepository mailings, IAdminMvpSettingsRepository mvpSettings, ISendEventRepository sendEvents, IAuditLogger auditLogger) : IAdminOperationService
{
    ...
    public AdminActionResult UpdateMvpSettings(AdminMvpSettings settings, string adminEmail, RequestMetadata request) { try { settings.EnsureValid(); var old = mvpSettings.Get(); var next = settings with { UpdatedAt = DateTimeOffset.UtcNow, UpdatedByAdminEmail = N(adminEmail) }; mvpSettings.Save(next); A(adminEmail, "admin_mvp_settings_changed", request, $"oldDailyLimit={old.DefaultDailySendLimit};newDailyLimit={next.DefaultDailySendLimit};oldRetention={old.ReplyBodyRetentionDays};newRetention={next.ReplyBodyRetentionDays}"); return AdminActionResult.Success(); } catch (Exception ex) when (ex is ArgumentException or ArgumentOutOfRangeException) { return AdminActionResult.Failure(ex.Message); } }
