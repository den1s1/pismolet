...
Workflow рассчитан на быстрый деплой статического сайта и нестабильные SSH-сессии shared-хостинга:

- старый незавершённый запуск отменяется при новом push;
- у job есть общий таймаут 3 минуты;
- SSH подключается по IPv4;
- для SSH задан `ConnectTimeout=8` и короткий keepalive;
- `rsync` делает до 2 попыток с паузой 10 секунд;
- у `rsync` задан таймаут передачи 30 секунд.

В нормальном случае деплой должен занимать несколько секунд. Если Timeweb временно не отвечает с GitHub runner, workflow быстро упадёт, а не будет висеть несколько минут. В таком случае можно перезапустить failed jobs вручную.
...
Единичные ошибки SSH-соединения с shared-хостингом допустимы: workflow делает короткую повторную попытку. Если обе попытки не прошли, можно повторно запустить failed jobs из GitHub Actions.
