...
- Sprint 5.6.6 - на ручной проверке: добавлен reader, который читает Postfix log с cursor-позиции, и админский ручной запуск reader-а.
...
Результат Sprint 5.6.6: приложение готово вручную читать `/var/log/mail.log` безопасно и инкрементально. Автозапуск по расписанию будет отдельным шагом после проверки прав доступа к log path и cursor path.
