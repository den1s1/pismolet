...
- Sprint 5.6.4 - на проверке: ingestion service применяет delivery status к `send_events`, если событие можно сопоставить по `ProviderMessageId = Postfix queue id`.
...
Ограничение текущего шага: для новых локальных SMTP-отправок ещё нужно начать сохранять Postfix queue id в `ProviderMessageId`. Без этого применение статусов будет работать только для событий, где queue id уже известен и сохранён как provider message id.
...
- извлекать queue id из SMTP-ответа вида `queued as ABC123`;
- сохранять queue id в `ProviderMessageId` для `LocalSmtp`;
- оставлять текущий fallback на `Message-ID`, если queue id не удалось получить;
- добавить тесты на извлечение queue id.

## Sprint 5.6.6 - UI delivery diagnostics
