...
- Sprint 5.6.5 - на проверке: локальный SMTP-адаптер извлекает Postfix queue id из ответа `queued as ...` и сохраняет его как provider message id, с fallback на `Message-ID`.
...
Результат Sprint 5.6.5: новые отправки через локальный Postfix должны получать `ProviderMessageId`, равный Postfix queue id. Это открывает путь к автоматическому сопоставлению строк `postfix/smtp` с `send_events`.
